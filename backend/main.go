...
func main() {
	...
	if cfg.GinMode == "release" {
		...
		if cfg.JWTSecret == "default_dev_secret_change_in_production" {
			log.Println("[WARN] Production configuration warning: JWT_SECRET is set to default development secret. Please configure JWT_SECRET on Render.")
		}
	} else {
	...
	// Top-level unauthenticated health check endpoint for Render / orchestrators
	...
	// 7. Configure HTTP server with timeouts listening on 0.0.0.0:<PORT>
	srv := &http.Server{
	...
	go func() {
		log.Printf("[INFO] LivePoll backend server listening on 0.0.0.0:%s", cfg.Port)
